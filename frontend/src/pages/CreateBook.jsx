...
const CreateBook = () => {
  ...
  const handleSubmit = () => {
    ...
    axios.post("http://localhost:5002/books/", data)
      .then(() => {
        setLoading(false)
    ...
    <div className="min-h-screen p-4">
      ...
      {loading ? (<LoadingUI />) :
        <div className="flex flex-col items-center">
          <h1 className="my-4 font-mono"> Create a New Book</h1>
          <div className="flex flex-col  border-2 border-sky-400 rounded-xl w-fit  p-4 bg-white">
            ...
            <button onClick={handleSubmit} className="mt-2 p-2 bg-sky-300 rounded-xl hover:bg-sky-500 transition duration-200" >
              {loading ? "Saving..." : 'Save'}
            </button>
          </div>
